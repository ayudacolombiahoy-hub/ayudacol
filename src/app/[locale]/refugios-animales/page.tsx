...
export default async function Pagina({
  ...
  const t = await getTranslations('refugios')
  const tMaps = await getTranslations('maps')
  const [refugios, municipios, perfil] = await Promise.all([
    listarRefugios(f),
    listarMunicipios(),
    obtenerPerfil(),
  ])
  ...
  return (
    <main className="mx-auto max-w-3xl p-6">
      <div className="mb-2 flex flex-wrap items-center justify-between gap-3">
        ...
        {esEquipo && (
          <Link href="/panel/refugios-animales" className="rounded-lg bg-slate-700 px-4 py-2 text-sm font-bold text-white hover:bg-slate-800">
            {t('gestionar')}
          </Link>
        )}
      </div>
          ...
          {refugios.map((a) => {
            ...
            return (
              <article key={a.id} className="rounded-lg border border-gray-200 bg-white p-4 shadow-sm">
                ...
                {a.especies && <p className="mt-1 text-sm text-gray-700">🐾 {t('especies')}: {a.especies}</p>}
                {libres !== null && (
