...
import { listarColaDesaparecidos } from '@/lib/datos/desaparecidos'
import { listarMunicipios } from '@/lib/datos/consultas'
import FilaDesaparecido from './FilaDesaparecido'
...
export default async function Pagina({ params }: { params: Promise<{ locale: string }> }) {
  ...
  const [cola, municipiosRaw] = await Promise.all([listarColaDesaparecidos(), listarMunicipios()])
  const mapaMuni = new Map(municipiosRaw.map((m) => [m.codigo_dane, `${m.nombre} — ${m.departamento}`]))

  return (
    <main className="mx-auto max-w-3xl p-6">
      <h1 className="mb-5 text-2xl font-extrabold">{t('desaparecidos.gestionar')}</h1>
      {cola.length === 0 ? (
        <p className="rounded-lg bg-gray-50 p-8 text-center text-gray-500">{t('desaparecidos.sinCola')}</p>
      ) : (
        <div className="grid gap-3">
          {cola.map((d) => <FilaDesaparecido key={d.id} d={d} municipio={mapaMuni.get(d.municipio_id)} />)}
        </div>
      )}
    </main>
  )
