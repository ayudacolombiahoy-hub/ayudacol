...
import { accionActualizarRefugio } from './acciones'
import { ESTADOS_REFUGIO } from '@/lib/validacion/esquemas'
...
export default function FilaRefugio({ a, municipio }: { a: Refugio; municipio?: string }) {
  const t = useTranslations('refugios')
  ...
  function guardarOcupacion() {
    ...
    start(async () => {
      const r = await accionActualizarRefugio(a.id, { ocupacion: n })
  ...
  function cambiarEstado(nuevo: string) {
    ...
    setEstado(nuevo)
    start(async () => {
      const r = await accionActualizarRefugio(a.id, { estado: nuevo })
  ...
  return (
    <article className="rounded-lg border border-gray-200 bg-white p-4">
      ...
      {a.especies && <p className="mt-1 text-sm text-gray-600">🐾 {t('especies')}: {a.especies}</p>}
